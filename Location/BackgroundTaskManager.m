...
-(UIBackgroundTaskIdentifier)beginNewBackgroundTask
{
    ...
    if([application respondsToSelector:@selector(beginBackgroundTaskWithExpirationHandler:)]){
        bgTaskId = [application beginBackgroundTaskWithExpirationHandler:^{
            NSLog(@"BackgroundTaskManager: background task %lu expired", (unsigned long)bgTaskId);
        }];
        if ( self.masterTaskId == UIBackgroundTaskInvalid )
        {
            self.masterTaskId = bgTaskId;
            NSLog(@"BackgroundTaskManager: started master task %lu", (unsigned long)self.masterTaskId);
        }
        else
        {
            //add this id to our list
            NSLog(@"BackgroundTaskManager: started background task %lu", (unsigned long)bgTaskId);
...
-(void)drainBGTaskList:(BOOL)all
{
    ...
    if([application respondsToSelector:@selector(endBackgroundTask:)]){
        ...
        for ( NSUInteger i=(all?0:1); i<count; i++ )
        {
            ...
            NSLog(@"BackgroundTaskManager: ending background task with id -%lu", (unsigned long)bgTaskId);
            [application endBackgroundTask:bgTaskId];
            [self.bgTaskIdList removeObjectAtIndex:0];
        }
        if ( self.bgTaskIdList.count > 0 )
        {
            NSLog(@"BackgroundTaskManager: kept background task id %@", [self.bgTaskIdList objectAtIndex:0]);
        }
        if ( all )
        {
            NSLog(@"BackgroundTaskManager: no more background tasks running");
            [application endBackgroundTask:self.masterTaskId];
            self.masterTaskId = UIBackgroundTaskInvalid;
        }
        else
        {
            NSLog(@"BackgroundTaskManager: kept master background task id %lu", (unsigned long)self.masterTaskId);
        }
    }
}
